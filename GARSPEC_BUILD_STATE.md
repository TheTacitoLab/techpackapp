...
- **PDF:** `@react-pdf/renderer` **4.5.1**, server-side `renderToStream` → streamed response (`lib/pdf/`); `sharp` downscales source images before embedding.
...
### PDF export
- Full document at `GET /products/{id}/techpack.pdf` (auth + workspace-scoped; `?layers=`, `?bom=0`, `?specs=0` filters); single composed page at `GET /products/{id}/pdf?pageId=` (inline preview). Both landscape A4, Helvetica, document-wide page numbering. **Neither route accepts a share token — no unauthenticated access path.** Both declare `maxDuration = 60` (hosts default to as little as 10 s), guard the WHOLE handler so any failure is a logged 500 rather than a throw, and the full-document route logs a one-line summary (pages, images embedded/skipped/KB, output KB, ms) on success.
...
- **Source images** (`lib/pdf/image-source.ts`, applied inside the shared fetcher in `page-data.ts`): uploads keep their original resolution, so every embedded raster is auto-oriented and downscaled before it becomes a data URI. The per-image ceiling is 2000 px on the longest edge (`PDF_IMAGE_MAX_EDGE`), but the document's images **share one pixel budget** (`imageEdgeFor`, 12 × 2000² total), so the edge falls as a product grows — 20 images → 1549 px, 60 → 894 px, floor 700 px. Format follows content: JPEG sources and anything costing more than 0.6 bytes/pixel as PNG go to JPEG (`PDF_IMAGE_JPEG_QUALITY`, default 80); transparent and line-art sources stay lossless PNG. The rule is a RATE, not a byte count — an absolute threshold flips shrunk photographs back to lossless (cost 34 MB on a 60-image pack in testing). SVG passes through untouched. An image is embedded as-is only when it is small in BOTH bytes and pixels. Fetch/resize runs 4-wide; a source over 50 MB is skipped. Every failure (incl. an unloadable `sharp`) degrades to the original bytes. **Why:** embedding 4500 px sources verbatim peaked at ~1.2 GB RSS for an 8-page pack — pdfkit decodes each PNG to a raw bitmap — which killed the export function outright (a 502 "function has crashed", not a catchable 500). Peak RSS is now ~350–600 MB and flat in page count.
- **Delivery is STREAMED** (`lib/pdf/response.ts`, `renderToStream`): a buffered serverless response is capped at 6 MB on Netlify and the platform serves its own error page past it — which is why big packs failed while small ones downloaded. A `ReadableStream` body raises the ceiling to 20 MB and skips `renderToBuffer`'s `Buffer.concat`. Layout completes before the stream is handed back, so a document that cannot be built still rejects in time for a clean 500. Worst case measured (dense line-art flats, which stay lossless): 8 pages 5.8 MB, 20 pages 10.3 MB, 60 pages 14.5 MB.
...
- ~~**WebP images unsupported in PDF**~~ — fixed: `lib/pdf/image-source.ts` transcodes WebP to PNG/JPEG on the way in, so WebP uploads now render (react-pdf still cannot embed WebP itself).
