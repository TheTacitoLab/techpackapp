...
- **Source images** (`lib/pdf/image-source.ts`, applied inside the shared fetcher in `page-data.ts`): uploads keep their original resolution, so every embedded raster is auto-oriented and downscaled to a 2000 px longest edge (`PDF_IMAGE_MAX_EDGE`) before it becomes a data URI. Format is chosen by content — JPEG sources and anything still heavy at 2000 px go to JPEG (`PDF_IMAGE_JPEG_QUALITY`, default 80), transparent and line-art sources stay lossless PNG. SVG passes through untouched. Fetch/resize runs 4-wide; a source over 50 MB is skipped. Every failure (incl. an unloadable `sharp`) degrades to the original bytes. **Why:** embedding 4500 px sources verbatim peaked at ~1.2 GB RSS for an 8-page pack — pdfkit decodes each PNG to a raw bitmap — which killed the export function outright (a 502 "function has crashed", not a catchable 500). Same pack now peaks around 360 MB.
