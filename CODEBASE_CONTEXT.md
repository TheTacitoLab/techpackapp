...
One system powers image annotation for all five layers: Colourways, Fabrics & Trim, Measurements, Construction, and Branding & Labels (0024).
...
`canvas_annotations` row: `{ id, slot_id, workspace_id, layer_type: canvas_layer_type, reference_code, x, y, pin_type: 'point'|'line', end_x, end_y, label_offset_x, label_offset_y, colourway_id, data: jsonb, created_by, created_at, updated_at }`. `x`/`y`/`end_x`/`end_y` are **0.0–1.0 fractions of the slot's FROZEN DESIGN-SPACE BOX** (since 0025; originally "fractions of the rendered size" per the `0013_canvas_schema.sql` header). `label_offset_x/y` (added in `0018_annotation_label_offset.sql`) are signed fractions for the draggable reference-code badge, relative to the pin tip.

**Frozen design-space box (0025 — the resize-drift fix):** `canvas_slots.lock_width/lock_height` record the slot's local rendered px size at LOCK time (captured by `lockSlot` from `FramingSlot`'s `getLocalBox()` — the same local-px space the framing pan `crop_x/crop_y` was authored in; cleared by `unlockSlot`/`fillSlot`). `AnnotationSlot` lays out the image (`paintedImageStyle`), pins (`fraction × design size`), capture overlays, and the colour sampler ALL against that fixed `lock_width × lock_height` box, then fits the box into the live container with one uniform centred `transform: scale(k)` (`k = min(liveW/lockW, liveH/lockH)`; the live size comes from a ResizeObserver on the slot container and drives ONLY `k`). Image and pins therefore share one frozen coordinate space and cannot drift on container resize (fullscreen portal swap, smaller dashboard render, future PDF). The verified geometry (`lib/cover-geometry.ts`, `coords.ts` `clientToFraction`) is untouched — only the box size fed to it changed; drag/click math already divides by `getBoundingClientRect()`, which absorbs the composed `stageZoom × k` scales. Null lock dims (never persisted post-0025-reset) fall back to the live size — the pre-0025 behaviour.

`canvas_layer_type` enum (15 values after 0024, `types/database.types.ts`): `fabric | trim | hardware | elastic | label_component | print | stitch | thread | packaging | measurement | construction_note | detail_callout | colourway | branding | label`. Note: `thread`, `packaging`, `detail_callout` exist in the DB enum but have **no owning UI layer yet** — schema headroom, not wired to anything. Four values are in the DB enum but NOT accepted for new pins (`RETIRED_LAYER_TYPES`): `hardware`/`elastic` — retired in the 0023 restructure (trims carry `data.trim_kind` instead; pre-0023 material pins cleared in that migration) — and `label_component`/`print` — the original reserved headroom superseded by 0024's cleanly-named `branding`/`label` (never used; `label_component` shares the L prefix with `label`, so blocking it guarantees one L sequence).

`LAYER_PREFIX` (`types/index.ts`) — single source of truth mapping every `canvas_layer_type` to its reference-code prefix letter: `fabric→F, trim→T, hardware→H, elastic→E, branding→B, label→L, label_component→L, print→P, stitch→S, thread→Th, packaging→Pk, measurement→M, construction_note→CN, detail_callout→DC, colourway→C` (entries for not-accepted values persist only because the record must stay total over the DB enum). The zod enum in `canvas-actions.ts` (`LAYER_TYPES`) is derived from `Object.keys(LAYER_PREFIX)` **minus `RETIRED_LAYER_TYPES`** (`hardware`/`elastic` retired in 0023; `label_component`/`print` superseded in 0024 — exported next to `LAYER_PREFIX`), so prefixes and accepted inputs can never drift apart.
...
- **`BrandingLabelAnnotationData`** (`types/index.ts`, 0024): `{ branding_type: BrandingType|null, label_type: LabelType|null, library_item_id, library_item_name, library_item_image_url, width_mm: number|null, height_mm: number|null, placement, colour, notes }`. Exactly one of the two type fields is set (matching `layer_type` = 'branding'/'label'); the library link is OPTIONAL (inherits inline quick-add; branding→`print_type`, label→`label_type` categories); B/L reference codes stay plain — the specific type is never encoded. Helpers in `components/canvas/branding-label-data.ts` (readers, type-label maps, `formatDimensionsMm`).
