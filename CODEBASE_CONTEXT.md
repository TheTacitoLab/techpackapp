...
`canvas_layer_type` enum (13 values, `types/database.types.ts:640-653`): `fabric | trim | hardware | elastic | label_component | print | stitch | thread | packaging | measurement | construction_note | detail_callout | colourway`. Note: `label_component`, `print`, `thread`, `packaging`, `detail_callout` exist in the DB enum but have **no owning UI layer yet** (see `components/canvas/layers.ts`, §3/§5) — they're schema headroom, not wired to anything.

`LAYER_PREFIX` (`types/index.ts:169-183`) — single source of truth mapping every `canvas_layer_type` to its reference-code prefix letter: `fabric→F, trim→T, hardware→H, elastic→E, label_component→L, print→P, stitch→S, thread→Th, packaging→Pk, measurement→M, construction_note→CN, detail_callout→DC, colourway→C`. The zod enum in `canvas-actions.ts` (`LAYER_TYPES`) is derived from `Object.keys(LAYER_PREFIX)`, so they can never drift apart.
...
- **`FabricTrimAnnotationData`** (`types/index.ts:129-141`): `{ library_item_id, library_item_name, category: LibraryCategory|null, composition, colour, gsm: number|null, placement, quantity: number|null, unit: 'per_metre'|'per_unit'|'per_kg'|null, supplier_code, notes }`.
...
`components/canvas/fabric-trim-pin-editor.tsx` (`FabricTrimPinEditor`) + `components/canvas/fabric-trim-data.ts`. Covers 4 `canvas_layer_type`s as one UI family (`fabric|trim|hardware|elastic`, mapped to library categories `fabric|trim|fastener|elastic` — note the `hardware`↔`fastener` name mismatch, intentional and documented). Sub-type is a segmented control, choosable only at creation (fixed forever after, since it determines the reference-code prefix). Selecting a `FabricPicker` (`components/fabric-picker.tsx`, a cmdk/Command+Popover combobox over `ResolvedLibraryItem[]`) item denormalises fields onto the annotation via `fabricTrimDataFromLibraryItem()` (composition, gsm, supplier_code, category) so the BOM never needs a join back to `library_items`. `readFabricTrimData()` falls back to legacy `{label}` for pre-this-session pins. This is the module `components/bom/bom-table.tsx` reads from — the BOM is a pure derived view (grouped by layer_type, reference-code sorted), **no manual add-row**, described as "a preview of the future exported PDF's BOM page."
...
### Master Library (Phase 3b)
`components/library-manager.tsx` (`LibraryManager`, Settings page) — CRUD over `library_items` with per-category dynamic property fields (`FIELD_CONFIG: Record<LibraryCategory, FieldDef[]>`), global-item hide/show toggle. Server actions in `app/(app)/settings/actions.ts`: `createLibraryItem`, `updateLibraryItem`, `deleteLibraryItem`, `toggleGlobalItem`. Resolved for consumption via `lib/library.ts`'s `getWorkspaceLibrary()`.
