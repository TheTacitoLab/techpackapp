...
/**
 * The collection twin of ProductLabels — same workspace label vocabulary
 * (managed in Settings → Labels), same chips + Command-popover picker,
 * writing to `collection_labels` instead of `product_labels`.
 */
export function CollectionLabels({
  ...
  function toggle(labelId: string) {
    ...
    startTransition(async () => {
      try {
        if (isAssigned) {
          await removeLabelFromCollection(collectionId, labelId);
        } else {
          await addLabelToCollection(collectionId, labelId);
        }
        router.refresh();
      } catch {
