-- ============================================================================
-- Phase 3b seed data only - run this if library_items table exists but is empty
-- Pure ASCII. Idempotent (WHERE NOT EXISTS guards). No dollar-quoting.
-- ============================================================================
...
-- ---- Category 3 - Fasteners: zips + buttons/snaps (carry colours) ------------
with cols as (
  select '[{"name":"Black","pantone_tcx":"19-4006 TCX","hex":"#2B2B2B"},{"name":"White","pantone_tcx":"11-0601 TCX","hex":"#F4F4F4"},{"name":"Navy","pantone_tcx":"19-4023 TCX","hex":"#2A3244"},{"name":"Volt","pantone_tcx":"13-0550 TCX","hex":"#C8F000"}]'::jsonb as c
)
insert into public.library_items (category, source, workspace_id, name, description, properties, is_active, created_by)
...
from (values
  ...
  ('Woven Loop Label','Satin woven loop label, sewn into side seam.','{"size_mm":"15x40mm","construction":"Satin woven","attachment":"Sew-in side seam","wash_fastness":"High"}'),
  ...
  ('Size Tab','Printed size tab sewn into main label.','{"size_mm":"15x15mm","construction":"Printed","attachment":"Sew into main label","wash_fastness":"High"}')
) as v(name, description, props)
where not exists (
  select 1 from public.library_items li
...
-- ---- Category 8 - Print & Decoration Types -----------------------------------
insert into public.library_items (category, source, workspace_id, name, description, properties, is_active, created_by)
select 'print_type'::public.library_category, 'global'::public.library_source, null,
       v.name, v.description, v.props::jsonb, true, null
from (values
  ('Screen Print (Plastisol)','Plastisol spot-colour screen print, separated layers.','{"artwork_format":"AI / EPS vector","colour_mode":"Spot (Pantone)","max_colours":8,"placement_notes":"Separated layers, no gradients"}'),
  ...
  ('Reflective Heat Transfer','Hi-vis reflective silver heat transfer.','{"artwork_format":"AI / PDF vector","colour_mode":"Grey/silver","max_colours":1,"placement_notes":"Hi-vis"}')
) as v(name, description, props)
where not exists (
  select 1 from public.library_items li
...
-- ---- Category 10 - Interlining & Interfacing ---------------------------------
insert into public.library_items (category, source, workspace_id, name, description, properties, is_active, created_by)
